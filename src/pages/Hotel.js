        ...
        <div className=''>
            <div className="bg-orange-400 rounded-full h-3/5 absolute w-3/5 -ml-5 rounded-tl-lg rounded-tr-lg ">
                <div className=" w-2/4">
                    ...
                    <Typography className='color-white '>Book Hotels & Homestays
                    </Typography>
                    <div className='ml-60'>
                        <Paper style={{ width: "220%" }} className=" h-96 mt-11 p-5 relative ">

                            <Typography>Where</Typography>

                            <TextField sx={{ mt: 3 }} fullWidth type='text'  placeholder='eg. - Area Landmark and Property Name'
                                value={hotelserach}
                                onChange={(e) => sethotelsearch(e.target.value)}
                                onClick={() => setOpenLocation(!openLocation)}
                            />
                            {openLocation && 
                            <Box className="shadow-md ring-offset-2 ring-opacity-50 rounded-lg overflow-y-scroll" sx={{ width: "400px", height: "350px", backgroundColor: "white", position: 'absolute', top: '125px', zIndex: '20', left: '30px' }}>
                                {hotelData && hotelData.map((item, index) => (
                                    <div className='p-5 hover:bg-blue-gray-50 font-rubik text-xl' key={index} onClick={() => { sethotelsearch(item.location), setOpenLocation(false) }}>
